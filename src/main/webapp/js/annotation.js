			...
			if (selectedServiceIF == "Nominatim") {
				...
				$.getJSON(requestUrl, function(results) {
					if (results.length == 0) {
						...
						//var filteredResults = filterByLocationtext(locationDescIF, results);
...
function filterByLocationtext(locationDesc, geocodingResults) {
	var polylineTypes = ['street', 'st', 'avenue', 'ave', 'road', 'rd', 'lane', 'ln', 'court', 'ct', 'drive', 'dr', 'boulevard', 'bld', 'blvd', 'highway', 'hwy', 'freeway', 'fwy', 'route', 'tollway', 'interstate', 'way', 'walk', 'passage', 'parkway', 'pkwy', 'bayou', 'river', 'creek']
	var abbreComplete = { 'st': 'street', 'ave': 'avenue', 'rd': 'road', 'ln': 'lane', 'ct': 'court', 'dr': 'drive', 'bld': 'boulevard', 'blvd': 'boulevard', 'hwy': 'highway', 'fwy': 'freeway', 'pkwy': 'parkway' }

	var reTypes = new RegExp('\\b(' + polylineTypes.join('|') + ')\\b', 'i');
	var abbres = Object.keys(abbreComplete);

	var locationDescProcessed = locationDesc.toLowerCase();
	locationDescProcessed = locationDescProcessed.replace(/[.,;:!?()[\]{}<>\/\\'"\-_…%#=|^~£€¥©®]+$/g, '');
	var match_Msg = locationDescProcessed.match(reTypes);
	if (match_Msg && abbres.includes(match_Msg[0])) {
		var fullType_Msg = abbreComplete[match_Msg[0]];
		locationDescProcessed = locationDescProcessed.replace(new RegExp('\\b' + match_Msg[0] + '\\b', 'gi'), fullType_Msg);
	}

	var filteredResults = [];
	for (var i = 0; i < geocodingResults.length; i++) {
		var placeName = geocodingResults[i]["display_name"].split(",")[0];
		placeName = placeName.trim();
		placeName = placeName.toLowerCase();
		placeName = placeName.replace(/[.,;:!?()[\]{}<>\/\\'"\-_…%#=|^~£€¥©®]+$/g, '');
		var match_PN = placeName.match(reTypes);
		if (match_PN && abbres.includes(match_PN[0])) {
			var fullType_PN = abbreComplete[match_PN[0]];
			placeName = placeName.replace(new RegExp('\\b' + match_PN[0] + '\\b', 'gi'), fullType_PN);
		}

		const editDistance = Levenshtein.get(locationDescProcessed, placeName);
		const maxLength = Math.max(locationDescProcessed.length, placeName.length);
		const similarity = 1 - (editDistance / maxLength);
		if (similarity >= 0.6) {
			filteredResults.push(geocodingResults[i]);
		}

		/*var similarity = stringSimilarity.compareTwoStrings(locationDescProcessed, placeName);
		if (similarity >= 0.8) {
			filteredResults.push(geocodingResults[i]);
		}*/

		/*var damerauLevenshteinDistance = stringSimilarity.damerauLevenshtein(locationDescProcessed, placeName);
		var maxLength = Math.max(locationDescProcessed.length, placeName.length);
		var normalizedDamerauLevenshteinDistance = 1 - (damerauLevenshteinDistance / maxLength);*/
	}
