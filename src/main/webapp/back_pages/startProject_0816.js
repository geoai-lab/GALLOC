...
divProjectList.on('click', 'a', function(event) {
	...
	if (aForProjectList.indexOf(event.target.id) != -1) {
		...
		$.ajax({
			url: 'ProjectServlet',
			type: 'GET',
			data: { action: "getProjectInfo", projName: encodeURIComponent(projName) },
			dataType: 'text',
			success: function(result, textStatus, jqXHR) {
				var resultObject = JSON.parse(result);
				if (resultObject.status == "success") {
...
function checkDataExists(projName) {
	return new Promise((resolve, reject) => {
		$.ajax({
			url: 'ProjectServlet',
			type: 'GET',
			data: { action: "isDataExists", projName: encodeURIComponent(projName) },
			dataType: 'text',
			success: function(result, textStatus, jqXHR) {
				var resultObject = JSON.parse(result);
				if (resultObject.status == "success") {
	...
	if (btnForAnnotateProjectList.indexOf(event.target.id) != -1) {
		...
		Promise.all([
			...
			.then(function(results) {
				const isDataExists = results[0];
				const isAnnotator = results[1];
				if (isDataExists && isAnnotator) {
					loadAnnotationPage(projName);
					isDataExists = false;
					isAnnotator = false;
				} else if ((!isAnnotator) & isDataExists) {
					var modalIsAnnotatorData = new bootstrap.Modal($("#modalIsAnnotatorData"));
					modalIsAnnotatorData.show();
					var message = "You are currently not an annotator for this project. Please first add yourself as an annotator before starting annotate by clicking <a role=\"button\" style=\"text-decoration: underline; color: blue;\" onmouseover=\"this.style.color='red';\" onmouseout=\"this.style.color='blue';\" id= \"a_ManageAnno_" + projName + "\">Manage annotator</a>";
					$("#modalBodyIsAnnotatorData").html("<div id=\"divAlertForIsAnnotatorData\" class=\"col-12\" style=\"font-size: 1rem;\">" + message + "</div>");
					return false;
				} else if ((!isDataExists) & isAnnotator) {
					var modalIsAnnotatorData = new bootstrap.Modal($("#modalIsAnnotatorData"));
					modalIsAnnotatorData.show();
					var message = "There is currently no data with this project. Please first upload data before starting annotate by clicking <a role=\"button\" style=\"text-decoration: underline; color: blue;\" onmouseover=\"this.style.color='red';\" onmouseout=\"this.style.color='blue';\" id= \"a_UpdateData_" + projName + "\">Update data</a>";
					$("#modalBodyIsAnnotatorData").html("<div id=\"divAlertForIsAnnotatorData\" class=\"col-12\" style=\"font-size: 1rem;\">" + message + "</div>");
					return false;
				} else {
					var modalIsAnnotatorData = new bootstrap.Modal($("#modalIsAnnotatorData"));
					modalIsAnnotatorData.show();
					var message = "You are currently not an annotator for this project, and there is also no data with it. Please first add yourself as an annotator and upload data before starting annotate by clicking <a role=\"button\" style=\"text-decoration: underline; color: blue;\" onmouseover=\"this.style.color='red';\" onmouseout=\"this.style.color='blue';\" id= \"a_ManageAnno_" + projName + "\">Manage annotator</a> and <a role=\"button\" style=\"text-decoration: underline; color: blue;\" onmouseover=\"this.style.color='red';\" onmouseout=\"this.style.color='blue';\" id= \"a_UpdateData_" + projName + "\">Update data</a>";
					$("#modalBodyIsAnnotatorData").html("<div id=\"divAlertForIsAnnotatorData\" class=\"col-12\" style=\"font-size: 1rem;\">" + message + "</div>");
					return false;
				}
			})
...
function loadAnnotationPage(projName) {
	...
	setTimeout(function() {
		...
		for (var i = 0; i < projectLists.length; i++) {
			if (projectLists[i].ProjectName == encodeURIComponent(projName)) {
				...
				$.ajax({
					...
					type: 'GET',
					data: { action: "getBatchMessages", projName: encodeURIComponent(projName), annotator: currentUser },
					dataType: 'text',
					success: function(result, textStatus, jqXHR) {
						var resultObject = JSON.parse(result);
						if (resultObject.status == "success") {
							...
							/*for (var i = 0; i < messages.length; i++) {
								var messageData = JSON.parse(messages[i].MessageData);
								messagesOneBatch.push(messageData);
							}
							$("#pMessageContent").html(messagesOneBatch[0].text);
							messagesOneBatch.shift();*/
						}
						else {
...
$("#btnDelProject").click(function() {
	...
	$.ajax({
		url: 'ProjectServlet',
		type: 'GET',
		data: { action: "delProject", projName: encodeURIComponent(currentProjName) },
		dataType: 'text',
		success: function(result, textStatus, jqXHR) {
			var resultObject = JSON.parse(result);
			if (resultObject.status == "success") {
...
$("#btnUploadDataHome").click(function() {
	...
	if (dataFile == undefined || dataFile.length == 0) {
		...
		corpus_data.append('projName', encodeURIComponent(projName));
...
$("#btnConfirmDelData").click(function() {
	...
	$.ajax({
		url: 'ProjectServlet',
		type: 'GET',
		data: { action: "deleteData", projName: encodeURIComponent(projName) },
		dataType: 'text',
		success: function(result, textStatus, jqXHR) {
			var resultObject = JSON.parse(result);
			if (resultObject.status == "success") {
...
$("#btnDLCorpusAll").click(function() {
	$.ajax({
		url: 'AnnotationServlet',
		type: 'GET',
		data: { action: "downloadAnnotationsAll", projName: encodeURIComponent(currentProjName) },
		dataType: 'text',
		success: function(result, textStatus, jqXHR) {
			var resultObject = JSON.parse(result);
			if (resultObject.status == "success") {
...
$("#btnDLCorpusOnlyResolved").click(function() {
	$.ajax({
		url: 'AnnotationServlet',
		type: 'GET',
		data: { action: "downloadAnnotationsResolved", projName: encodeURIComponent(currentProjName) },
		dataType: 'text',
		success: function(result, textStatus, jqXHR) {
			var resultObject = JSON.parse(result);
			if (resultObject.status == "success") {
				...
				var link = document.createElement('a');
				link.href = URL.createObjectURL(blob);
				link.download = currentProjName + '_Resolve.json';
				link.click();
				URL.revokeObjectURL(link.href);
			}
			else {
...
divProjectList.on('click', 'button', function(event) {
	if (btnForCheckStatusList.indexOf(event.target.id) != -1) {
		...
		$.ajax({
			url: 'AnnotationServlet',
			type: 'GET',
			data: { action: "checkStatus", projName: encodeURIComponent(projName) },
			dataType: 'text',
			success: function(result, textStatus, jqXHR) {
				var resultObject = JSON.parse(result);
				if (resultObject.status == "success") {
					...
					statusHtml += '<li>Messages that need to be resolved: ' + msgResolvedOrSame + '</li>';
