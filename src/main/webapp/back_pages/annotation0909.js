					...
					if (results.length == 0) {
						...
						if (onlyOneType) {
							if (flattedCoorsList[0].type == "LineString") {
								var selectedIndex = fingdingClosetOne(flattedCoorsList, mapIF.getCenter());
...
$('#btnPointType').click(function() {
	...
	var geometryFoundMap = L.GeoJSON.geometryToLayer({ type: "Point", coordinates: [geocodingResultsUsing[0].lon, geocodingResultsUsing[0].lat] });
...
function fingdingClosetOne(flattedSpatialFoorprint, currentViewPoint) {
