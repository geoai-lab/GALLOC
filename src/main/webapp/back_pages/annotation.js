...
$('#pMessageContent').on('click', '.bi-x-circle-fill', function(event) {
	...
	var annotatedSpanIDList = allAnnotatedSpanID.map(item => item.spanID);
	var selectedSpanIDList = selectedSpansIndex.map(item => item.spanID);
	if (annotatedSpanIDList.includes(closingSpanID)) {
		for (var i = 0; i < allAnnotatedSpanID.length; i++) {
			if (allAnnotatedSpanID[i].spanID == closingSpanID) {
			...
			if (selectedService == "Nominatim") {
				...
				$.getJSON(requestUrl, function(results) {
					if (results.length == 0) {
						...
						if (results[0].geojson.type == "LineString") {
							returnedGeometryType = "Polyline";
						} else {
							returnedGeometryType = results[0].geojson.type;
						}
						if (spatialTypeSelected == returnedGeometryType) {
							/*mapGeoCoords = { type: "FeatureCollection", features: [] };
							var multiGeometryCoords = [];
							for (var i = 0; i < geocodingResults.length; i++) {
								mapGeoCoords.features.push({ type: "Feature", geometry: geocodingResults[i].geojson, properties: {} });
								var geometryFoundMap = L.GeoJSON.geometryToLayer(geocodingResults[i].geojson);
								geometryFoundMap.addTo(drawnItemsForAnnotations);
								multiGeometryCoords.push(geocodingResults[i].geojson.coordinates);
							}

							var multiGeometry = {};
							if (geocodingResults[0].geojson.type == "LineString") {
								multiGeometry = {
									type: "MultiLineString",
									coordinates: multiGeometryCoords
								};
							} else if (geocodingResults[0].geojson.type == "Polygon") {
								multiGeometry = {
									type: "MultiPolygon",
									coordinates: multiGeometryCoords
								};
							...
							if (geocodingResults[0].geojson.type == "LineString") {
								...
								for (var i = 0; i < geocodingResults.length; i++) {
									mapGeoCoords.features.push({ type: "Feature", geometry: geocodingResults[i].geojson, properties: {} });
									var geometryFoundMap = L.GeoJSON.geometryToLayer(geocodingResults[i].geojson);
									geometryFoundMap.options.id = generateRandomStringID();
									geometryFoundMap.addTo(drawnItemsForAnnotations);
									multiGeometryCoords.push(geocodingResults[i].geojson.coordinates);
								}
								...
								multiGeometry = geocodingResults[0].geojson;
								mapGeoCoords.features.push({ type: "Feature", geometry: multiGeometry, properties: {} });
...
$('#btnPointType').click(function() {
	...
	mapGeoCoords = {
		type: "FeatureCollection", features: [{ type: "Feature", geometry: { type: "Point", coordinates: [geocodingResults[0].lon, geocodingResults[0].lat] }, properties: {} }]
	};
	var geometryFoundMap = L.GeoJSON.geometryToLayer({ type: "Point", coordinates: [geocodingResults[0].lon, geocodingResults[0].lat] });
	geometryFoundMap.addTo(drawnItemsForAnnotations);
	mapForAnnotations.setView(geometryFoundMap.getLatLng(), 12);
})
...
$('#btnReturnedType').click(function() {
	...
	if (geocodingResults[0].geojson.type == "LineString") {
		for (var i = 0; i < geocodingResults.length; i++) {
			multiGeometryCoords = multiGeometryCoords.concat(geocodingResults[i].geojson.coordinates);
		}
		multiGeometryCoords.sort(compareCoordinates);
		mapGeoCoords.features.push({ type: "Feature", geometry: { type: "LineString", coordinates: multiGeometryCoords }, properties: {} });
		var geometryFoundMap = L.GeoJSON.geometryToLayer({ type: "LineString", coordinates: multiGeometryCoords });
		geometryFoundMap.addTo(drawnItemsForAnnotations);
		mapForAnnotations.fitBounds(geometryFoundMap.getBounds());
	} else if (geocodingResults[0].geojson.type == "Polygon") {
		var geometryFoundMap = L.GeoJSON.geometryToLayer(geocodingResults[0].geojson);
		geometryFoundMap.addTo(drawnItemsForAnnotations);
		mapForAnnotations.fitBounds(geometryFoundMap.getBounds());
		mapGeoCoords = { type: "FeatureCollection", features: [{ type: "Feature", geometry: geocodingResults[0].geojson, properties: {} }] };
	}*/
})
